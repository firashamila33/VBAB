...
$query=$conn->query(" SELECT messages.id_membre AS m1 , messages.message_membre AS m2 , user.id_user AS m3 , user.nom AS m4 FROM messages INNER JOIN user
  ON messages.id_membre = user.id_user  WHERE ((messages.id_membre=".$_SESSION['id']." AND messages.id_dest ='{$idmembre}') OR (messages.id_dest=".$_SESSION['id']." AND messages.id_membre='{$idmembre}'))ORDER BY temps DESC ");
//var_dump($query);
$messages=array();
